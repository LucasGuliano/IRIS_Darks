...
class dark_times:
    ...
    def __init__(self,time,
        ...
        email@email.org
        /Volumes/IRIS_Darks/IRIS_LEVEL1_DARKS/
    ...
    def dark_query(self):
    #use drms module to download from JSOC (https://pypi.python.org/pypi/drms) which is the default
        client = drms.Client(email=self.email)
        fmt = '%Y.%m.%d_%H:%M'
    ...
    def download_par(self,index):
        # get file from JSOC
        outf = self.expt.download(self.bdir,index=index,fname_from_rec=True)
        #format output file 

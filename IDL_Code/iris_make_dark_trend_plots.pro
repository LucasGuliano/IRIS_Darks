pro iris_make_dark_trend_plots
    set_plot,'Z'
...
; loop and plot each port individually
    ...
    ;        oplot,jime,port,psym=syms[i],color=color[i]
            port = gropave[j,*]
            porte = gropsig[j,*]
            oplot,groptim,port,psym=syms[j],color=color[j],thick=3
            errplot,groptim,port-porte,port+porte,color=color[j],thick=3
   ;overplot long term trend
   ;        oplot,groptim,groptrd[i,*],color=color[i],psym=0,linestyle=lines[i]
            oplot,upanarray,offsets[j,*],color=color[j],psym=0,linestyle=lines[j],thick=4

        endfor

        al_legend,['port1','port2','port3','port4'],psym=syms,colors=color,linestyle=[0,0,0,0],box=0,/top,charsize=2.0
        al_legend,['fit port1','fit port2','fit port3','fit port4'],psym=[0,0,0,0],colors=colors,linestyle=lines,box=0,/bottom,/right,charsize=2.0

        write_png,pdir+'/'+type[i]+'_trend_plot.png',tvrd(/true)

...
#set up print, refit, and quit buttons
    ...
    def iris_dark_set(self):
        ...
        for i in ptype: 
            ...
            ''' print('FUV4 Data:')
            for entry in range(len(self.fdata['fuv4'][1])):
                print(datetime.fromtimestamp(284014800+time[entry]).strftime("%m/%d/%Y %I:%M"))
                print(self.fdata['fuv3'][1][entry])'''
...
################################################################################################################       
    ...
    def offset(self,dt0,amp1,amp2,p1,phi1,phi2,lin,quad,off):
        ...
        trend[model_E_start] = E_trend[model_E_start]
        
        ################################################################################################################
        #                       MODEL F STARTING 2025/01/01                                          #
        ################################################################################################################
        ...
        #Trend from 2025/01/01 forward 
        trend[model_F_start] = F_trend[model_F_start]

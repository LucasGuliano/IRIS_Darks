...
if not keyword_set(year_list) then year_list = ['2014','2015','2016','2017','2018', '2019', '2020', '2021', '2022', '2023', '2024', '2025', '2026'] 
...
if not keyword_set(file_loc) then file_loc = '/Volumes/IRIS_Darks/opabina/level0/simpleB/'

if keyword_set(sav_file) and file_test(sav_file) then restore, sav_file 
if keyword_set(sav_file) and file_test(sav_file) then print, 'RESTORED '+sav_file
...
for ii=0, n_elements(year_list)-1 do begin
	for kk=0, n_elements(month_list)-1 do begin
		...
		if file_test(file_loc+year_list[ii]+'/'+month+'/') then begin ;Only do it if the files actually exist
            ...
            ;FIXED TO NOT BE PATHDEPENDENT, ONLY FILE FORMAT DEPENDENT (WHICH DOESN"T CHANGE)
            ;days = STRMID(files,60,2)
            file_split = strsplit(files[0], '/',/EXTRACT)
            file_part = file_split[-1]
            days = strmid(file_part, 9, 2)
...
;           days = days[uniq(days)]
            for pp=0,n_elements(days)-1 do begin
        		...
        		if total(strmatch(identifier, this_identifier)) eq 0 then begin ;Only do the analysis if it has not been done before. 
                    ...
                    ;Should it be '*.fits' instead of '_*.fits'? Doesn't seem to matter 
                    ...
                    ;Added Jakub Prchlik 09/15/2016
                                ;concatenate two arrays only if the
                                ;next day array has some length

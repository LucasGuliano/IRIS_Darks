...
#Only use python3 formatting for tkinter package
...
class gui_dark(Tk.Frame):
    ...
    def __init__(self,parent):
        ...
        V5 NOTES:
        Model E covers 01/2024 through 10/31/2024 (11 and 12 covered in F for better model agreement)
        
        V6:
        Model F covers November 1st, 2024 to early 2026.
        
        V6:
        Updated to be compatible with Python3
        
        ***Please see the "README" file for instruction on how to create a new model segment.***
        ...
        MODEL D: November 1st, 2022 to December 31st, 2023
        MODEL E: January 1st, 2024 to November 1st, 2024
        MODEL F: November 1st, 2024 to.......
...
###################    Model E    #####################
        ...
        for i,line in enumerate(Efile):
            ...
            if i == 0: self.plis=line.split(',')
                ...
                sline = line.split('=')
                #create parameter lis for dictionary input
                self.E_dict[sline[0]] = [float(j) for j in sline[1].split(',')]
        #close parameter file
        Efile.close()   

###################    Model F    #####################
        #When adding new model, change this dict from current_dict to LETTER_dict
        Ffile = open('Model_F_Parameters.txt','r')
        #read input parameters from file
...
#######################################################################
        ...
        #dictionary of when to start the quadratic term for the fit (Only used in Model A)
        self.dtq0 = {}
        self.dtq0['fuv'] = 5.e7 #02/23/2015 
        self.dtq0['nuv'] = 7.e7 #10/05/2015

        #dictionary of when to end the quadratic term for the fit (Only used in Model A)
        self.dtq1 = {}
        ...
        #                       11/01/2024                              #
        #################################################################
        self.seperation_F = 1.4464188e+09 #11/01/2024 12:00
        #self.seperation_F = 1.4064188e+09
        ################################################
...
#set the information based on screen size (Can modify scale to better fit your screen if needed)
...
#set up print, refit, and quit buttons
        ...
        #Setup box to set the initial parameter range
        ...
        for m,i in enumerate(self.current_keys):
            ...
            for c,j in enumerate(self.current_dict[i]):
                ...
                #create input text
                #CAN CHANGE BOX SIZE HERE TO FIT TO SCREEN BETTER, CHANGE WIDTH VALUE
                self.ivar[i+'_'+self.plis[c]+'_min'] = Tk.Entry(frame,textvariable=inp_min,width=9)
                self.ivar[i+'_'+self.plis[c]+'_med'] = Tk.Entry(frame,textvariable=inp_val,width=9)
                self.ivar[i+'_'+self.plis[c]+'_max'] = Tk.Entry(frame,textvariable=inp_max,width=9)
    ...
    def iris_dark_plot(self):
        ...
        for i in sorted(self.fdata.keys()):
            ...
            # OR expanded plot further out to next cycle to show where data is going
...
################################################################################################################       
    ...
    def offset(self,dt0,amp1,amp2,p1,phi1,phi2,lin,quad,off):
        ...
        ####################### PARAMETERS FOR MODEL E ######################
        #Read in the the Model E parameters (DO NOT ADJUST PARAMETERS)
        E_amp1 = self.E_dict[port][0]
        E_amp2 = self.E_dict[port][1]
        E_p1 = self.E_dict[port][2]
        E_phi1 = self.E_dict[port][3]
        E_phi2 = self.E_dict[port][4]
        E_lin = self.E_dict[port][5]
        E_quad = self.E_dict[port][6]
        E_off = self.E_dict[port][7]
        
        ###################################################################################
        ##### NOTE THAT THE CURRENT MODEL PARAMETERS WILL NOT HAVE A LETTER PREFIX ########
        ###################################################################################
        ...
        trend[tuple(post_bo)] = (trend+drop_trend_offset+incs_trend_amplit)[tuple(post_bo)]
        ...
        trend[tuple(post_ns)] = (trend+incr_trend_offset)[tuple(post_ns)]
        ...
        trend[tuple(model_B_start)] = B_trend[tuple(model_B_start)]
        ...
        trend[tuple(model_C_start)] = C_trend[tuple(model_C_start)]
        ...
        trend[tuple(model_D_start)] = D_trend[tuple(model_D_start)]
        ...
        #New trend takes the same form as the orignal model, just with new parameter values
        E_trend = (E_amp1*np.sin(c*(dt0/E_p1+E_phi1)))+(E_amp2*np.sin(c*(dt0/(E_p1/2.)+E_phi2)))+(E_lin*(dt0))+(E_quad*(dt0**2.00))+(E_off)
        
        #Trend from 2024/01/01 forward 
        trend[tuple(model_E_start)] = E_trend[tuple(model_E_start)]
        
        ################################################################################################################
        #                       MODEL F STARTING 2024/11/01                                          #
        ################################################################################################################
        model_F_start = [(dt0 >= self.seperation_F - self.t0dict[self.cport])]
        #New trend takes the same form as the orignal model, just with new parameter values
        F_trend = (amp1*np.sin(c*(dt0/p1+phi1)))+(amp2*np.sin(c*(dt0/(p1/2.)+phi2)))+(lin*(dt0))+(quad*(dt0**2.00))+(off)
        
        #Trend from 2024/11/01 forward
        trend[tuple(model_F_start)] = F_trend[tuple(model_F_start)]
        ...
        #Return trend of all the models
        return trend
    ...
    def refit(self):
        ...
        for i in self.refit_list:
            ...
            #### This does the actual calculation of new model parameters ###
            ...
            #temporary line plot
            #ptim = np.linspace(self.fdata[i][0].min(),self.fdata[i][0].max()+1e3,500)
            # OR plot forward for next cycle to show where data is going
            ptim = np.linspace(self.fdata[i][0].min(),self.fdata[i][0].max()+4e7,500)
              
            t_line, = self.wplot[i[:-1]].plot(ptim,self.offset(ptim,*popt),linestyle='--',color=self.fdata[i][3])
            self.canvas.draw()
            #Force Tkinter to draw the temporary before reaching the mainloop where it would normally update all plots (when this function ends)
